  ...
  return (
    <header className="_nav px-2 sm:px-0">
      <div className="_header sm:flex h-full">
        ...
        <div className="w-full sm:w-[240px] xl:w-[320px] py-4 px-1 shadow-2xl sm:p-0 _header_loc flex items-center sm:justify-center cursor-pointer sm:hover:bg-gray-50">
          <LocationPicker />
        </div>
        <div style={{display:'flex',alignItems: 'center'}} className="flex-1 relative shadow-xl _header_search">
          <SearchBox />
        </div>
        ...
        <div onClick={toggleCartModal} className="py-2 hidden md:flex h-full items-center mr-8 ml-3">
          <CartButton />
        </div>
      </div>
      {isCartOpen && <CartPanel toggleCartModal={toggleCartModal} />}
    </header>
  );
};
